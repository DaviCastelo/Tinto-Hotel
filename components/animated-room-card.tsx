...
export function AnimatedRoomCard({ room, index }: AnimatedRoomCardProps) {
  ...
  return (
    ...
    >
      {/* Imagem com animação independente */}
      <div className={`relative ${isEven ? 'lg:order-1' : 'lg:order-2'} transition-all duration-700 delay-200 ${
      ...
      <div className={`space-y-6 ${isEven ? 'lg:order-2' : 'lg:order-1'} transition-all duration-700 delay-300 ${
        isVisible ? 'opacity-100 transform translate-y-0' : 'opacity-0 transform translate-y-6'
      }`}>
        <div>
